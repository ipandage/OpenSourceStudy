    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        setData: function (data)
        {
            ...
            if (p.columns)
            {
                ...
                for (var j = 0, len = p.columns.length; j < len; j++)
                {
...
(function ($)
{
    ...
    $(".l-dialog-winbtns .l-dialog-close").live('mouseover', function ()
    {
    ...
    l.controls.Dialog.ligerExtend(l.core.Win, {
        ...
        _render: function ()
        {
            var g = this, p = this.options;
            g.set(p, true);
            //var dialog = $('<div class="l-dialog"><table class="l-dialog-table" cellpadding="0" cellspacing="0" border="0"><tbody><tr><td class="l-dialog-tl"></td><td class="l-dialog-tc"><div class="l-dialog-tc-inner"><div class="l-dialog-icon"></div><div class="l-dialog-title"></div><div class="l-dialog-winbtns"><div class="l-dialog-winbtn l-dialog-close"></div></div></div></td><td class="l-dialog-tr"></td></tr><tr><td class="l-dialog-cl"></td><td class="l-dialog-cc"><div class="l-dialog-body"><div class="l-dialog-image"></div> <div class="l-dialog-content"></div><div class="l-dialog-buttons"><div class="l-dialog-buttons-inner"></div></div></td><td class="l-dialog-cr"></td></tr><tr><td class="l-dialog-bl"></td><td class="l-dialog-bc"></td><td class="l-dialog-br"></td></tr></tbody></table></div>');
            ...
            //设置事件
            $(".l-dialog-close", g.dialog).click(function ()
            {
                if (p.isHidden)
                    g.hide();
                else
        ...
        _setImage: function ()
        {
            ...
            if (p.type)
            {
            	if (p.type == 'success' || p.type == 'donne' || p.type == 'ok')
                {
                    ...
                    g.dialog.content.css({ paddingLeft: 64, paddingBottom: 10, paddingTop:20, paddingRight:10, overflow:'hidden', height:'48px', lineHeight:'25px'});
                }
    ...
    $.ligerDialog.alert = function (content, title, type, callback, ico)
    {
    ...
    $.ligerDialog.warning = function (content, title, callback, ico)
    {
        if (typeof (title) == "function")
        {
            callback = title;
    ...
    $.ligerDialog.success = function (content, title, onBtnClick, ico)
    {
        return $.ligerDialog.alert(content, title, 'success', onBtnClick, ico);
    };
    $.ligerDialog.error = function (content, title, onBtnClick, ico)
    {
        return $.ligerDialog.alert(content, title, 'error', onBtnClick, ico);
    };
    $.ligerDialog.warn = function (content, title, onBtnClick, ico)
    {
        return $.ligerDialog.alert(content, title, 'warn', onBtnClick, ico);
    };
    $.ligerDialog.question = function (content, title, ico)
    {
        return $.ligerDialog.alert(content, title, 'question', ico);
    };


    $.ligerDialog.prompt = function (title, value, multi, callback, ico)
    {
        ...
        */
        ...
        _onClick: function (e)
        {
            ...
            if (src.checkboxall) //复选框全选
            {
                ...
                {
                    if (uncheck)
                        g.unselect(g.records[rowid], obj);
                    else
                        g.select(g.records[rowid], obj);
                }
        ...
        unselect: function (rowParm, obj)
        {
            ...
            g._removeSelected(rowdata);
            g.trigger('unSelectRow', [rowdata, rowid, rowobj, obj]);
        },
...
(function ($)
{
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        moveToPrevTabItem: function ()
        {
            ...
            for (var i = 0, l = leftList.length - 1; i < l; i++)
            {
...
(function ($)
{
    ...
    $.ligerDefaults.Tree = {
        ...
        childrenName: 'children'  // 子节点键名, add by fanshine124@gmail.com in 2013.1.31
    };
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        hasChildren: function (treenodedata)
        {
            ...
            return treenodedata[this.options.childrenName] ? true : false;
        },
        //获取父节点 数据
        ...
        loadData: function (node, url, param, ischecked)
        {
            var g = this, p = this.options;
            // g.loading.show();
            ...
            $.ajax({
                ...
                success: function (data)
                {
                    if (!data) return;
                    g.loading.hide();
                    g.append(node, data, ischecked);
                    g.trigger('success', [data, node, param]);
                },
                error: function (XMLHttpRequest, textStatus, errorThrown)
                {
                    try
        ...
        append: function (parentNode, newdata, ischecked, nearNode, isAfter)
        {
            ...
            if (parentNode == null)//增加到根节点
            {
                ...
                // edit by fanshine124@gmail.com in 2013.1.31 below
                $(".l-body", g.tree).hover(function ()
                {
                    $(this).addClass("l-over");
                    // if(p.onHoverNode) p.onHoverNode(true, this);
                }, function ()
                {
                    $(this).removeClass("l-over");
                    // if(p.onHoverNode) p.onHoverNode(false, this);
                });
                // edit by fanshine124@gmail.com in 2013.1.31 upwords
                g._upadteTreeWidth();
        ...
        arrayToTree: function (data, id, pid)      //将ID、ParentID这种数据格式转换为树格式
        {
            ...
            for (var i = 0; i < itemLength; i++)
            {
                ...
                parentData[p.childrenName] = parentData[p.childrenName] || [];
                parentData[p.childrenName].push(currentData);
            }
            return targetData;
        },
        //根据数据索引获取数据
        _getDataNodeByTreeDataIndex: function (data, treedataindex)
        {
            ...
            for (var i = 0; i < data.length; i++)
            {
                if (data[i].treedataindex == treedataindex)
                    return data[i];
                if (data[i][p.childrenName])
                {
                    var targetData = g._getDataNodeByTreeDataIndex(data[i][p.childrenName], treedataindex);
        ...
        _setTreeDataStatus: function (data, status)
        {
            ...
            $(data).each(function ()
            {
                ...
                if (this[p.childrenName])
                {
                    g._setTreeDataStatus(this[p.childrenName], status);
                }
            });
        },
        //设置data 索引
        _addTreeDataIndexToData: function (data)
        {
            ...
            $(data).each(function ()
            {
                ...
                if (this[p.childrenName])
                {
                    g._addTreeDataIndexToData(this[p.childrenName]);
                }
            });
        },
        _addToNodes: function (data)
        {
            ...
            if ($.inArray(data, g.nodes) == -1)
                g.nodes.push(data);
            if (data[p.childrenName])
            {
                $(data[p.childrenName]).each(function (i, item)
                {
                    g._addToNodes(item);
                });
            }
        ...
        _appendData: function (treeNode, data)
        {
            ...
            if (treenodedata && !treenodedata[p.childrenName] ) treenodedata[p.childrenName] = [];
            $(data).each(function (i, item)
            {
                if (treenodedata)
                    treenodedata[p.childrenName][treenodedata[p.childrenName].length] = item;
                else
        ...
        _upadteTreeWidth: function ()
        {
            ...
            if (p.treeWidth) treeWidth = p.treeWidth; // add by fanshine124@gmail.com
        ...
        _getTreeHTMLByData: function (data, outlineLevel, isLast, isExpand)
        {
            ...
            for (var i = 0; i < data.length; i++)
            {
                ...
                // edit by fanshine124@gmail.com in 2013.1.31 below
                var nodeText = '<span class="cliptext">' + o[p.textFieldName] + '</span>'; // default
                if(p.nodeText) nodeText = p.nodeText(o); // add by user
                treehtmlarr.push(nodeText + '</div>');
                // edit by fanshine124@gmail.com in 2013.1.31 upwards
                if (g.hasChildren(o))
                {
                    var isLastNew = [];
                    ...
                    if(o[p.childrenName]) treehtmlarr.push(g._getTreeHTMLByData(o[p.childrenName], outlineLevel + 1, isLastNew, isExpandCurrent).join(''));
                }
                treehtmlarr.push('</li>');
            }
            treehtmlarr.push("</ul>");
        ...
        _getDataByTreeHTML: function (treeDom)
        {
            ...
            $("> li", treeDom).each(function (i, item)
            {
                ...
                if ($("> ul", this).length > 0)
                {
                    data[dataindex][p.childrenName] = g._getDataByTreeHTML($("> ul", this));
                }
            });
            return data;
        },
        ...
        _setTreeEven: function ()
        {
            ...
            g.tree.click(function (e)
            {
                ...
                {
                    if(p.onBeforeCheck && !p.onBeforeCheck({data:treenodedata,target:treeitem[0]})){
                        return false;
                    }
                ...
                g.tree.ligerDrag({ revert: true, animate: false,
                    ...
                    onStopDrag: function (current, e)
                    {
                        ...
                        if (g.nodeDropIn != -1)
                        {
                            ...
                            {
                                var children = nodes[i][p.childrenName];
                                if (children)
                                {
